...
//            "</tr>" +
        ...
        initContent: function (editor) {
            ...
            if (lang) {
                var html = jQuery.parseTmpl(this.tpl, lang.static);
            }
            this.root().html(html);
        },
        initEvent: function (editor, $w) {
            ...
            if(link){
                jQuery('#edui-link-Jhref',$w).val(utils.html(jQuery(link).attr('href')));
                jQuery('#edui-link-Jtitle',$w).val(jQuery(link).attr('title'));
                jQuery(link).attr('target') == '_blank' && jQuery('#edui-link-Jtarget').attr('checked',true)
            }
        },
        buttons: {
            'ok': {
                exec: function (editor, $w) {
                    var href = jQuery('#edui-link-Jhref').val().replace(/^\s+|\s+$/g, '');

                    if (href) {
                        editor.execCommand('link', {
                            'href': href,
                            'target': jQuery("#edui-link-Jtarget:checked").length ? "_blank" : '_self',
                            'title': jQuery("#edui-link-Jtitle").val().replace(/^\s+|\s+$/g, ''),
                            '_href': href
                        });
                    }
                }
