...
<div id="classtree_top" class="classtree-topbar">
 ...
 <?php if($_G['adminid']==1) { ?><a class="import " href="<?php echo ADMINSCRIPT;?>?mod=orguser&op=import"  title="导入用户"></a><?php } ?>
...
"themes" : {"responsive" : true,"variant":'large'},
"data":{
dataType:'json',
url : (BROWSER.ie && BROWSER.ie<9)?'<?php echo ADMINSCRIPT;?>?mod=orguser&op=ajax&do=getchildren&t='+new Date().getTime():'<?php echo ADMINSCRIPT;?>?mod=orguser&op=ajax&do=getchildren',
data : function (node) {
  return { 'id' : node.id };
}
  }
 ...
 if(inst.is_disabled(node)) return false;
 else return true;
 }
  },
...
jQuery.getJSON('<?php echo ADMINSCRIPT;?>?mod=orguser&op=ajax&do=create',{'forgid':obj.parent,'orgid':obj.id,'t':new Date().getTime()},function(json){
...
var inst = jQuery.jstree.reference(data.reference),
obj = inst.get_node(data.reference);
jQuery.getJSON('<?php echo ADMINSCRIPT;?>?mod=orguser&op=ajax&do=create',{'forgid':obj.id,'t':new Date().getTime()},function(json){
if(!json || json.error){
showmessage(json.error,'danger',3000,1);
}else if(json.orgid>0){
var arr={"id":json.orgid,"text":json.orgname,"type":"organization","icon":(json.forgid>0)?'dzz/system/images/department.png':'dzz/system/images/organization.png'};
...
if(confirm(msg)){
jQuery.getJSON('<?php echo ADMINSCRIPT;?>?mod=orguser&op=ajax&do=delete&t='+new Date().getTime(),data,function(json){
if(json.error){
showmessage(json.error,'danger',3000,1);
}else{
if(inst.is_selected(obj)) {
...
moderator_add(orgid,uid);
 ...
 //console.log(post);
 if(needsave){
jQuery.getJSON('<?php echo ADMINSCRIPT;?>?mod=orguser&op=ajax&do=move&t='+new Date().getTime(),post,function(json){
if(json.error){
showmessage(json.error,'danger',3000,1);
inst.move_node(data.node,data.parent,data.position);
}
});
 }
 });
...
jQuery.getJSON('<?php echo ADMINSCRIPT;?>?mod=orguser&op=ajax&do=rename&t='+new Date().getTime(),{'orgid':data.node.id,text:data.text},function(json){
if(!json || json.error){
obj.text=data.old;
inst.set_text(obj,data.old);
