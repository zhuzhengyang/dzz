...
@ob_end_flush();
     ...
     */
    function backup($tablename = '', $dir, $size,$folder) {
        $dir = $dir ? $dir : './backup/';
        // 创建目录
        if (! is_dir ( $dir )) {
        ...
        if (! empty ( $tablename )) {
            ...
            while ( $record = mysql_fetch_array ( $data ) ) {
                // 单条记录
                $sql .= $this->_insert_record ( $tablename, $num_fields, $record,$tablefields );
                // 如果大于分卷大小，则写入文件
            ...
            while ( $table = mysql_fetch_array ( $tables ) ) {
                // 获取表名
                $tablename = $table [0];
				if(in_array($tablename,array('dzz_admincp_session','dzz_district'))) continue;
                // 获取表结构
                $sql .= $this->_insert_table_structure ( $tablename );
                $data = mysql_query ( "select * from " . $tablename );
                $num_fields = mysql_num_fields ( $data );
     ...
     * @return string
     */
    private function _insert_record($table, $num_fields, $record,$tablefields) {
        // sql字段逗号分割
        $insert = '';
        $comma = "";
        ...
        for($i = 0; $i < $num_fields; $i ++) {
			...
			$insert .= $comma.(!empty($record[$i]) && (strexists($tablefields[$i]['Type'], 'char') || strexists($tablefields[$i]['Type'], 'text')) ? '0x'.bin2hex($record[$i]) : '\''.mysql_escape_string($record[$i]).'\'');
            $comma = ",";
        }
        $insert .= ");" . $this->ds;
        return $insert;
    ...
    private function _import($sqlfile) {
       ...
       // $sqls = array ();
		...
		$sqlquery = $this->_splitsql($sqldump);
	...
	private function _splitsql($sql) {
		...
		foreach($queriesarray as $query) {
			...
			foreach($queries as $query) {
				$ret[$num] .= ($query[0] == "#" || $query[0] == "-") ? NULL : $query;
			}
			$num++;
		}
		return($ret);
