...
<div style="font-size: 16px;font-weight:bold;text-align:center;padding: 0 0 25px 0;text-shadow:1px 1px 1px #fff">协同办公平台</div><div style="font-size: 12px;text-align:center;padding: 0 0 10px 0;text-shadow:1px 1px 1px #fff">@2013-2014 DzzOffice</div><div style="font-size: 12px;text-align:center;text-shadow:1px 1px 1px #fff">备案信息</div>');
		//添加ftp方式
	...
	$setarr[]=array('domain'=>'localhost',
	 ...
	 $setarr[]=array('domain'=>'localhost',
	  ...
	  $setarr[]=array('domain'=>'localhost',
		...
		//删除不需要的应用
		if($appid=DB::result_first("select appid from ".DB::table('app_market')." where appurl='{adminscript}?mod=member'")){
			 C::t('app_market')->delete_by_appid($appid);
		}
		//修改机构管理应用为机构用户管理
		if($appid=DB::result_first("select appid from ".DB::table('app_market')." where appurl='{adminscript}?mod=organization'")){
			 C::t('app_market')->update($appid,array('appname'=>'机构和用户管理','appurl'=>'{adminscript}?mod=orguser','group'=>2,'notdelete'=>1,'position'=>1));
		}
		//添加图标管理应用
		...
		if($i>=$count_i) {
			show_msg('部门数据升级完成，进入下一步操作', $theurl.'?step=delete');
		}
		if($orgid=DB::fetch_first("select orgid from %t where 1 order by orgid limit $i,1 ",array('organization',$i))){
...
function show_footer() {
	...
	<div id="footer">Copyright © 2012-2014 DzzOffice.com All Rights Reserved.</div>
	</div>
	<br>
	</body>
	</html>
